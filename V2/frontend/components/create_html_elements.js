    ...
    if (quantity > 0) {
      container.innerHTML = `
        <div class="cart-quantity-wrapper">
          <button id="minus-btn-${item_id}" class="qty-btn qty-btn-minus">&minus;</button>
...
async function addHTMLToBody(
        ...
        const main_card = `
        <div class="qb-card">
          <button id="${buttonId}" class="qb-card-icon" style="background:none;border:none;padding:0;cursor:pointer;width:100%;">
            <img src="${imageSrc}" alt="${title}" onerror="this.style.display='none'">
          </button>
          <div class="qb-card-body">
            <div class="qb-card-top">
              <h2 class="qb-card-title">${title}</h2>
              <span class="qb-card-price">₹${price}</span>
            </div>
            <p class="qb-card-desc">${description}</p>
            ...
            <img src="${imageSrc}" alt="${title}"
              style="${
                image_type == "landscape"
                  ? 'width: clamp(20rem, 50vw, 40rem); height: clamp(14rem, 24vw, 26rem); object-fit: cover;'
                  : 'width: clamp(14rem, 30vw, 22rem);  height: clamp(14rem, 30vw, 22rem); object-fit: cover;'
              }">
          </div>
          <h2>${title}</h2>
          <p>${description}</p>
          <button class="add-to-cart" style="margin-top: 0.5rem; padding: 0.5rem 1rem; color: white; border: none; border-radius: 5px; cursor: pointer;">
            Add to token
          </button>
        </div>
        ${main_card}
